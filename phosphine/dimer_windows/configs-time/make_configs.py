...
#dose_times = [6,60,600,6000,60000]
dose_times = [600000]

for time in dose_times:
    os.mkdir('dose%s'%time)
    os.chdir('dose%s'%time)
    for dimer_window in [2,2.5,3,3.5,4]:

        f = open('%sdimconfig_highT.py'%dimer_window,'w')
        f.write(""" #
...
final_index = -1
time_to_check_after = %s  #In units of s
...
"""%(int(math.ceil(dimer_window)),dimer_window,time))
